...
public class ViewRunner {
    ...
    public static ComponentConfiguration runConfiguration(final Configuration[] configuration) {
        final TemplateConfiguration tmplConfig = getTemplateConfiguration(configuration);
        if (tmplConfig != null) {
            final IFragment root = tmplConfig.get(ENTRY_TEMPLATE);
            final ComponentConfiguration rootConfig = root.getConfiguration(ENTRY_TEMPLATE, null);
    ...
    private static ComponentConfiguration initializeConfigurations(final ComponentConfiguration rootConfig,
        ...
        // TODO [HIGH] initiate configs
        return rootConfig;
    }

    /**
     * gets the templateconfiguration out of an configurationlist
     * 
     * @param configuration
     * @return
     */
    public static TemplateConfiguration getTemplateConfiguration(final Configuration[] configuration) {
        for (final Configuration config : configuration) {
            if (ConfigurationType.templateconfiguration.equals(config.getConfigurationType())) {
                return (TemplateConfiguration) config;
            }
        }
        return null;
    }
     ...
     * @param configuration
     */
    public static void runView(final OutWriterImplementationAdapter out, final ComponentConfiguration configuration) {
